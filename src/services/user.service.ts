...
export const getProfileService = async (id_usuario: number): Promise<Usuario | null> => {
    const query = `
        ...
        u.facebook, u.twitter, u.instagram, u.web, u.sobre_mi, u.experiencia
        FROM tblc_usuario u
...
export const updateProfileService = async (id: number, usuario: Omit<Usuario, 'id'>): Promise<void> => {
    const query = `
        UPDATE tblc_usuario SET id_municipio = ?, correo = ?, telefono = ?,
        telefono_urgencias = ?, whatsapp = ?, direccion = ?, facebook = ?,
        twitter = ?, instagram = ?, web = ?, sobre_mi = ?, experiencia = ? WHERE id_usuario = ?
    `;
    await db.query(query, [
        usuario.id_municipio, usuario.correo, usuario.telefono, usuario.telefono_urgencias,
